...
export async function createAttachmentPresignedUrl(userId: string, todoId: string): Promise<string> {
  logger.info(`Create attachment presigned url for (userId, todoId) = (${userId}, ${todoId})`);
  
  await checkTodoExist(userId, todoId);
  const presignedUrl: string = await attachmentUtils.createAttachmentPresignedUrl(todoId);
  logger.info(`Created presigned URL: ${presignedUrl}`);

  const urlObj = new URL(presignedUrl);
  urlObj.search = ''; //remove query string
  const updatedTodo: TodoItem = await todosAccess.updateTodoAttachmentUrl(userId, todoId, urlObj.toString());
  if (updatedTodo) {
...
export async function checkTodoExist(userId: string, todoId: string): Promise<void> {
